...
const ItemSelectionSearch = ({
 ...
 const handleSelection = (key) => {
   if (items.has(key)) {
     ...
     setSelectedItems(new Set([...selectedItems].filter((item) => item !== key)));
   } else {
     ...
     <div className="inputContainer">
       ...
       {dropDown && value && (
         <ul className="itemList">
           {itemList
             .filter((item) => item.ID.toLowerCase().includes(value.toLowerCase()))
             .slice(0, maxItems)
             .map((item) => (
               <li key={item.ID} className="listButton" onClick={() => handleSelection(item.ID)}>
                 {item.ID}
               </li>
             ))}
         </ul>
     ...
     <div className="selectedItemsContainer">
       {[...selectedItems].map((item) => (
         <div key={item} className="selectedItem">
           {item}
           <div className="xButtonContainer" onClick={() => handleDeletion(item)}>
